...
public class TBUProcessJournalARInvoiceUnBilled extends SvrProcess  {
	...
	protected String doIt() throws Exception {
		...
		SQLgetDoctype.append(" AND Name = 'Operation Production' ");
		...
		SQLGetBPAcct.append(" (SELECT C_Receivable_Unbilled_Acct ");
		SQLGetBPAcct.append(" FROM C_BP_Customer_Acct ");
     	...
     	ResultSet rsLine = null;
			try {
				...
				while (rsLine.next()) {
					...
					journalLine.setAmtSourceCr(price);
					journalLine.setAmtAcct(Env.ZERO, price);
				...
				if(journalLine != null) {
					...
					journalLine.setAccount_ID(Account_ID);
					journalLine.setAmtSourceDr(SumGrandTotal);
					journalLine.setAmtSourceCr(Env.ZERO);
					journalLine.setAmtAcct(SumGrandTotal, Env.ZERO);
