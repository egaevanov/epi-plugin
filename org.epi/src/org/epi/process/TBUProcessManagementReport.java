...
public class TBUProcessManagementReport extends SvrProcess {
	...
	protected void prepare() {
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null);
				...
				p_C_Year_ID  = (int)para[i].getParameterAsInt();
			}
			else
	...
	protected String doIt() throws Exception {
		...
		SQLExecFucntion.append("SELECT adempiere.f_rptmgm_get_monthly_value(?,?,?,?,?)");
		boolean IsOK = true;
			...
			if(IsOK) {
				...
				if(report != null) {
					...
					if(AD_Process_ID > 0) {
						...
						list.add (new ProcessInfoParameter("C_Year_ID", p_C_Year_ID, null, null, null));
