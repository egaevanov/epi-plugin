 ...
 */
public abstract class EPICreateFromInvoice extends CreateFrom
{
	/**
	 *  Protected Constructor
	 *  @param mTab MTab
	 */
	public EPICreateFromInvoice(GridTab mTab)
	{
		super(mTab);
	 ...
	 * @param C_BPartner_ID
	 */
	protected ArrayList<KeyNamePair> LoadShipmentData (int C_BPartner_ID, int InvoiceOrg)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT s.M_InOut_ID,").append(display)
			...
			+ "WHERE s.C_BPartner_ID=? AND s.IsSOTrx=? AND s.AD_Org_ID = ? AND s.DocStatus IN ('CL','CO')"
			+ " AND s.M_InOut_ID IN "
			...
			if(!isSOTrx)
				sql.append(" LEFT OUTER JOIN M_MatchInv mi ON (sl.M_InOutLine_ID=mi.M_InOutLine_ID) "
					...
					+ " WHERE s2.C_BPartner_ID=? AND s2.IsSOTrx=? AND s2.AD_Org_ID = ? AND s2.DocStatus IN ('CL','CO') "
	 ...
	 */
	protected ArrayList<KeyNamePair> LoadRMAData(int C_BPartner_ID, int InvoiceOrg) {
		ArrayList<KeyNamePair> list = new ArrayList<KeyNamePair>();
...
//						}
	...
	protected ArrayList<KeyNamePair> LoadOrderData (int C_BPartner_ID, boolean forInvoice, boolean sameWarehouseOnly, boolean forCreditMemo,int invoiceOrg)
	{
		ArrayList<KeyNamePair> list = new ArrayList<KeyNamePair>();
		...
		StringBuffer sql = new StringBuffer("SELECT o.C_Order_ID,")
			...
			.append(colBP)
			.append("=? AND o.IsSOTrx=? AND o.AD_Org_ID = ? AND o.DocStatus IN ('CL','CO') AND o.C_Order_ID IN (SELECT ol.C_Order_ID FROM C_OrderLine ol WHERE ");
		if (forCreditMemo)
